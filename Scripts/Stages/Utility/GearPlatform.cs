...
public partial class GearPlatform : Node2D
{
    ...
    [Export] public float ProngLength = 28f;   // total height centered at y=0; reduced from before
    ...
    public override void _Draw()
    {
        ...
        // Flat rectangle barrel — same size as the collision box
        Color barrelColor = DarkColor.Lerp(FaceColor, 0.6f);
        DrawRect(new Rect2(-_hw, -bodyHalfH, Width, BodyHeight), barrelColor);
        // Subtle lighter band across the middle for a bit of depth
        DrawRect(new Rect2(-_hw, -bodyHalfH * 0.3f, Width, bodyHalfH * 0.6f), FaceColor * 0.6f);

        // --- Prongs drawn as outlined rectangles on top of the barrel ---
        // Collect visible prongs and sort by cosA ascending so farther ones draw first.
        ...
        for (int i = 0; i < ProngCount; i++)
        {
            ...
            if (cosA <= 0f) continue; // back face — never draw, even with bias
        ...
        foreach (var (cosA, sinA, halfH) in prongs)
        {
            ...
            // Brightness scales with cosA so near-center prongs are brighter
            ...
            // Bright top cap and dark bottom cap for depth
